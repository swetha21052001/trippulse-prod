...
def search_hotels(location: str, check_in: str, check_out: str, budget_limit: Optional[float] = None) -> List[HotelOption]:
    ...
    if cache_key in HOTEL_CACHE:
        ...
        if cached and location.lower().strip() in cached[0].location.lower():
            return cached

    db = get_firestore_client()
    if db:
        try:
            doc_ref = db.collection("hotel_cache").document(cache_key)
            doc = doc_ref.get()
            if doc.exists:
...
def search_hotels(location: str, check_in: str, check_out: str, budget_limit: Optional[float] = None) -> List[HotelOption]:
    ...
    if db:
        try:
            db.collection("hotel_cache").document(cache_key).set({"hotels": [h.dict() for h in hotels]})
        except Exception:
            pass
